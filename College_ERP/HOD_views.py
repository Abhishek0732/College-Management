...
def ADD_STUDENT(request):
    ...
    if request.method=='POST':
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            student.save()
            messages.success(request, user.first_name + ' ' + user.last_name + ' ' + ' Are Saved Succesfully')
            return redirect('add_student')
...
def UPDATE_STUDENT(request):
    ...
    if request.method == 'POST':
        ...
        sesssion_year = Session_Year.objects.get(id=session_year_id)
        student.session_year_id = sesssion_year
...
def ADD_STAFF(request):
    if request.method == 'POST':
        ...
        if CustomUser.objects.filter(username = username).exists():
            ...
            staff.save()
            messages.success(request,'Staffs are Successfully Added')
            return redirect('add_staff')
...
def DELETE_STAFF(request,admin):
    ...
    staff.delete()
    messages.success(request,'Recod are Successfully Deleted!')
    return redirect('view_staff')
...
def STAFF_SEND_NOTIFICATION(request):
    staff = Staff.objects.all()
    see_notificatiion = Staff_Notification.objects.all().order_by('-id')[0:5]

    context = {
        'staff':staff,
        'see_notificatiion':see_notificatiion
    }
